...
const LOCAL_ASSETS = '../minimal/assets/minecraft/1.21.1/';

Promise.all([
  fetch(`${LOCAL_ASSETS}items.json`).then(r => r.json()),
  fetch(`${LOCAL_ASSETS}block_definition.json`).then(r => r.json()),
  fetch(`${LOCAL_ASSETS}model.json`).then(r => r.json()),
  Promise.resolve({}),
  fetch(`${LOCAL_ASSETS}item_components.json`).then(r => r.json()),
  fetch(`${LOCAL_ASSETS}atlas.json`).then(r => r.json()),
  new Promise<HTMLImageElement>(res => {
    const image = new Image();
    image.onload = () => res(image);
    image.crossOrigin = 'Anonymous';
    image.src = `${LOCAL_ASSETS}atlas.png`;
  })
